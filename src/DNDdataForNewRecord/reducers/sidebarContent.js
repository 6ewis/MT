...
export default function(state = {}, action) {
 switch (action.type) {
  ...
  case DROP_ITEM:
    const isPayloadDropItem = R.anyPass([R.propEq('id', action.payload.id), R.propEq('entity_type', action.payload.value)]);
    const removePayloadAttr = R.when(isPayloadDropItem, R.dissoc(action.payload.attribute));
    return R.map(removePayloadAttr, state);
  case CLICK_X:
    const isPayloadClickX = R.anyPass([R.propEq('id', action.payload.id), R.propEq('entity_type', undefined)]);
